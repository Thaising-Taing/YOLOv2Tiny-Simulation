...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.Run= customtkinter.CTkButton(self.left_frame, text="Run Training", command=self.Run_click, width=button_width, height=button_height)
        self.Run .place(x=10, y=200)
        
        self.Stop = customtkinter.CTkButton(self.left_frame, text="Run Inference", command=self.Stop_click, width=button_width, height=button_height)
        ...
        text_label = customtkinter.CTkLabel(self.right_frame_1, text="Layer Debug", font=("Helvetica", 15))
    ...
    def Run_click(self):      
        ...
        if Mode == "Training":
            ...
            s = time.time()
            Weight_Dec, Bias_Dec, Beta_Dec, Gamma_Dec, Running_Mean_Dec, Running_Var_Dec = PreProcessing.WeightLoader()
            e = time.time()
            ...
            train_dataloader = DataLoader(train_dataset, batch_size=8, shuffle=True, num_workers=2, collate_fn=detection_collate, drop_last=True)
            
            # Small Training Dataset
            small_dataset = torch.utils.data.Subset(train_dataset, range(0, 80))
            # print("Sub Training Dataset: " + str(len(small_dataset)))
            s = time.time()
            small_dataloader = DataLoader(small_dataset, batch_size=8, shuffle=True, num_workers=2, collate_fn=detection_collate, drop_last=True)
            e = time.time()
            print("Data Loader : ",e-s)

            iters_per_epoch = int(len(small_dataset) / 8)
            ...
            for epoch in range(Total_epoch):
                ...
                train_data_iter = iter(small_dataloader)
                for step in range(iters_per_epoch):
                    im_data, gt_boxes, gt_classes, num_obj = next(train_data_iter)

                    Image_1_start = time.time()    
                    
                    # YOLOv2TinyFPGA = YOLOv2_Tiny_FPGA(Weight_Dec=Weight_Dec, Bias_Dec=Bias_Dec, 
                    #                         Beta_Dec=Beta_Dec, Gamma_Dec=Gamma_Dec,
                    #                         Running_Mean_Dec=Running_Mean_Dec, 
                    #                         Running_Var_Dec=Running_Var_Dec,
                    #                         Image=im_data) 

                    YOLOv2TinyFPGA = YOLOv2_Tiny_FPGA(Weight_Dec, Bias_Dec, 
                                            Beta_Dec, Gamma_Dec,
                                            Running_Mean_Dec, 
                                            Running_Var_Dec,
                                            im_data,
                    ...
                    s = time.time()
                    YOLOv2TinyFPGA.Write_Weight()       
                    e = time.time()
                    print("Write Weight Time : ",e-s)

                    s = time.time()
                    YOLOv2TinyFPGA.Write_Image()
                    e = time.time()
                    ...
                    s = time.time()
                    Loss = YOLOv2TinyFPGA.Forward(gt_boxes=gt_boxes, gt_classes=gt_classes, num_boxes=num_obj)
                    e = time.time()
                    print("Forward Process Time : ",e-s)
                    self.change_color_red()

                    s = time.time()
                    YOLOv2TinyFPGA.Backward()
                    e = time.time()
                    ...
                    s = time.time()
                    Weight_Dec, Beta_Dec, Gamma_Dec, Bias_Dec = YOLOv2TinyFPGA.Weight_Update(learning_rate)
                    e = time.time()
                    print("Weight Update Time : ", e-s)
                    Image_1_end = time.time()
                    print("1 Image Train Time : ",Image_1_end-Image_1_start)
                ...
                # Save Pickle: 
                if epoch % 1 == 0:
                    _data = Weight_Dec, Bias_Dec, Beta_Dec, Gamma_Dec, Running_Mean_Dec, Running_Var_Dec, epoch
                    output_file = os.path.join(output_dir, f'Params_{epoch}.pickle')
                ...
                self.output_text = f"Epoch: {epoch+1}/{Total_epoch}--Loss: {Loss}"
                print(f"Epoch: {epoch+1}/{Total_epoch}--Loss: {Loss}")
                self.Show_Text(self.output_text)
                print(f"Epoch: {epoch+1}/{Total_epoch}--Loss: {Loss}")

        #     Loss_list.append(Loss.item())
        # plt.plot(range(1, Total_epoch + 1), Loss, linestyle='-')
        # plt.xlabel('Epoch')
        # plt.ylabel('Hardware Loss')
        # plt.title('Loss Curve')
        # plt.grid(True)
        # plt.show()
        # plt.savefig('Hardware_Loss_8Images.png')
        else:
            Params_Pickle_Path = os.path.join(output_dir, f'Params_{58}.pickle')
            with open(Params_Pickle_Path, 'rb') as handle:
                b = pickle.load(handle)
            Weight_Dec, Bias_Dec, Beta_Dec, Gamma_Dec, Running_Mean_Dec, Running_Var_Dec, epoch = b
            ...
            for epoch in range(Inference_Epochs):             
                inference_data_iter = iter(small_val_dataloader)
                for step in range(iters_per_epoch):
                # for step in range(1):    
                    im_data, im_info = next(inference_data_iter)
                    # im_data, gt_boxes, gt_classes, num_obj = next(inference_data_iter)
                    
                    # Intialization the YOLOv2_Tiny Class: 
                    YOLOv2TinyFPGA = YOLOv2_Tiny_FPGA(Weight_Dec=Weight_Dec, Bias_Dec=Bias_Dec, 
                                                Beta_Dec=Beta_Dec, Gamma_Dec=Gamma_Dec,
                                                Running_Mean_Dec=Running_Mean_Dec, 
                                                Running_Var_Dec=Running_Var_Dec,
                    ...
                    for i in range (0, len(microcode)):
                        ...
                        self.bar.write(0x0, 0x00000012) # wr en
                        self.bar.write(0x0, 0x00000010) # wr en low
                    # YOLOv2TinyFPGA.Forward(gt_boxes=None, gt_classes=None, num_boxes=None)
                    ...
                    output_data = YOLOv2TinyFPGA.Forward_Inference(gt_boxes=None, gt_classes=None, num_boxes=None)
                    
                    # Running the Detection and Drawing Boundary Boxes
                    detection(yolo_outputs=output_data, epoch=epoch, small_val_dataloader=small_val_dataloader, val_imdb=val_imdb)
...
class YOLOv2_Tiny_FPGA(object):
    ...
    def Write_Weight(self):
        ...
        if Weight_Converted:
            Weight_Bfloat, Bias_Bfloat, Beta_Bfloat, Gamma_Bfloat, Running_Mean_Bfloat, Running_Var_Bfloat = PreProcessing.Weight_Converted_Func(self.Weight_Dec, self.Bias_Dec, self.Beta_Dec, 
                                                                                                                                                            self.Gamma_Dec, self.Running_Mean_Dec, self.Running_Var_Dec)  
            s = time.time()
            Weight_1st_Layer0 = New_Weight_Hardware_ReOrdering_Layer0(16, 16, Weight_Bfloat[0], ['0000']*16, ['0000']*16, ['0000']*16, Iteration="1")
            Weight_1st_Layer1 = New_Weight_Hardware_ReOrdering_OtherLayer(32, 16, Weight_Bfloat[1], ['0000']*32, ['0000']*32, ['0000']*32, Iteration="1")
            Weight_1st_Layer2 = New_Weight_Hardware_ReOrdering_OtherLayer(64, 32, Weight_Bfloat[2], ['0000']*64, ['0000']*64, ['0000']*64, Iteration="1")
            Weight_1st_Layer3 = New_Weight_Hardware_ReOrdering_OtherLayer(128, 64, Weight_Bfloat[3], ['0000']*128, ['0000']*128, ['0000']*128, Iteration="1")
            Weight_1st_Layer4 = New_Weight_Hardware_ReOrdering_OtherLayer(256, 128, Weight_Bfloat[4], ['0000']*256, ['0000']*256, ['0000']*256, Iteration="1")
            Weight_1st_Layer5 = New_Weight_Hardware_ReOrdering_OtherLayer(512, 256, Weight_Bfloat[5], ['0000']*512, ['0000']*512, ['0000']*512, Iteration="1")
            Weight_1st_Layer6 = New_Weight_Hardware_ReOrdering_OtherLayer(1024, 512, Weight_Bfloat[6], ['0000']*1024, ['0000']*1024, ['0000']*1024, Iteration="1")
            Weight_1st_Layer7 = New_Weight_Hardware_ReOrdering_OtherLayer(1024, 1024, Weight_Bfloat[7], ['0000']*1024, ['0000']*1024, ['0000']*1024, Iteration="1")
            Weight_1st_Layer8 = New_Weight_Hardware_ReOrdering_Layer8(128, 1024, Weight_Bfloat[8], Bias_Bfloat)
            e = time.time()
