...
echo "      transfer address/size:  $offset"
...
if [ $h2cChannels -gt 0 ]; then
  ...
  for ((i=0; i<=3; i++))
  do
    addrOffset=$(($transferSize * $i+ $offset))
    curChannel=$(($i % $h2cChannels))
    echo "Info: Writing to h2c channel $curChannel at address offset $addrOffset."
    $tool_path/dma_to_device -d /dev/xdma0_h2c_${curChannel} -f data/weight0_convert_bin.bin -s $transferSize -a $addrOffset -c $transferCount & #datafile${i}_4K.bin -s $transferSize -a $addrOffset -c $transferCount &
...
total_time=$(echo "scale=6; $end_time - $start_time" | bc)
echo "Total time: $total_time seconds (s)"
...
if [ $c2hChannels -gt 0 ]; then
  ...
  for ((i=0; i<=3; i++))
  do
    addrOffset=$(($transferSize * $i+$offset1))
    ...
    $tool_path/dma_from_device -d /dev/xdma0_c2h_${curChannel} -f data/output_weight0_convert_bin.bin -s $transferSize -a $addrOffset -c $transferCount & #output_datafile${i}_4K.bin -s $transferSize -a $addrOffset -c $transferCount &
    # If all channels have active transactions we must wait for them to complete
...
if [ $h2cChannels -eq 0 ]; then
  ...
  for ((i=0; i<=3; i++))
  do
    cmp data/output_weight0_convert_bin.bin data/weight0_convert_bin.bin -n $transferSize
    returnVal=$?
    if [ ! $returnVal == 0 ]; then
