...
def Fmap_Reverse_Ordering(Out_CH, Out_Size, DataCH0_List, DataCH1_List):
    ...
    rearrangeCH_ar = np.repeat('0000', A * 2).reshape(int(Out_CH / 16), Out_Size * (Out_Size + iter_13 * 3), 16)
    ...
    convert_ar = convert_ar.reshape(Out_CH, (Out_Size + iter_13 * 3), Out_Size)

    final_ar = np.repeat('0000', Out_CH * Out_Size * Out_Size).reshape(Out_CH, Out_Size, Out_Size)
    for i in range(0, Out_CH):
        for j in range(0, iter_13):
            for l in range(0, Out_Size):
...
def Weight_Gradient_Hardware_ReOrdering(Out_CH, In_CH, DataCH0_List, DataCH1_List):
  ...
  filter_ar = np.repeat(zero, origin_size).reshape(int(Filter_Num/8), 2, 4, int(In_Channel_Num/4), kenel_size, 4)
  ...
  convert_ar = np.repeat(zero, Filter_Num*In_Channel_Num*kenel_size).reshape(int(Filter_Num/8), 2, 4, int(In_Channel_Num/4), 4, kenel_size) # Using "2" make 8 outch
  for fn in range (0,int(Filter_Num/8)):
    for fc2 in range (0,2):
      for fc1 in range (0,4):
