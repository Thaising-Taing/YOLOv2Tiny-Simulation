...
def ReLu_Marking_Ordering(Out_CH, Out_Size, DataCH0_List, DataCH1_List):
    ...
    convert_ar0 = origin_ar0.reshape(origin_size//2, 2, 16)
    convert_ar1 = origin_ar1.reshape(origin_size//2, 2, 16)
...
def Fmap_Reverse_Ordering(Out_CH, Out_Size, DataCH0_List, DataCH1_List):
    ...
    def bfloat16_to_decimal(bfloat16_data):
        # NumPy 배열에서 bfloat16 데이터를 binary 데이터로 변환

        # hex_string = format(int(bfloat16_data, base=16), '04x')


        binary_data = np.binary_repr(int(bfloat16_data, 16), width=16)

        # IEEE 754 표준에 따라 부동 소수점으로 변환
        sign_bit = int(binary_data[0])
        exponent = int(binary_data[1:9], 2) - 127
        fraction = 1 + int(binary_data[9:], 2) / (2 ** 7)

        # 부호 비트를 적용하여 부동 소수점 값을 얻습니다.
        # decimal_value = (-1) ** sign_bit * fraction * (2 ** exponent)
        if int(binary_data) != 0:
            decimal_value = (-1) ** sign_bit * fraction * (2 ** exponent)
        else :
            decimal_value = 0
...
def Weight_Gradient_Hardware_ReOrdering(Out_CH, In_CH, DataCH0_List, DataCH1_List):
    ...
    # bfloat16_array = np.array(origin_ar, dtype=np.uint16)  # bfloat16 데이터
    decimal_array = np.vectorize(bfloat16_to_decimal)(convert_ar)
...
def Weight_Gradient_Hardware_ReOrdering_Layer0(Out_CH, In_CH, DataCH0_List, DataCH1_List):
    ...
    convert_ar0 = origin_ar0.reshape(int(half_size/(kenel_size*8)),kenel_size*8,16)
    convert_ar1 = origin_ar1.reshape(int(half_size/(kenel_size*8)),kenel_size*8,16)
    ...
    convert_ar = convert_ar.reshape(Filter_Num, In_Channel_Num, kenel_size)
    final_ar = np.repeat(zero, Filter_Num*3*kenel_size).reshape(Filter_Num, 3, kenel_size)

    # final_ar[:][:][:] =  convert_ar[0:16][0:3][0:9]
    for i in range (0, Filter_Num):
        final_ar[i][0] = convert_ar[i][0]
        final_ar[i][1] = convert_ar[i][1]
        final_ar[i][2] = convert_ar[i][2]

    final_ar = final_ar.reshape(Filter_Num*3*kenel_size)
