...
# print(pytorch_model.params['W1'][0, 0, 0, 0:3])
...
if __name__ == '__main__':
    ...
    # pytorch_model.save_pickle           = True  # Save output in form of pickle file

    # # pytorch_model.save_layer_output = True  # Save output for each layers (not module)
    # pytorch_model.save_module_output    = True  # Save output for each module

    # pytorch_model.save_in_dec_format    = True  # Save output for each module in decimal format
    # pytorch_model.save_in_hex_format    = True  # Save output in hex format

    
    # Fout, Fcache, loss, loss_grad, BlDout, Bgrads = pytorch_model.train(im_data, gt_boxes=gt_boxes,
    #                                                                     gt_classes=gt_classes, num_boxes=num_obj)
    def get_dataset(datasetnames):
        names = datasetnames.split('+')
        dataset = RoiDataset(get_imdb(names[0]))
        print('load dataset {}'.format(names[0]))
    ...
    # small_dataset = torch.utils.data.Subset(train_dataset, range(0, 16))
    # # small_dataset = torch.utils.data.Subset(train_dataset, range(0, ))
    # batch_size=8
    # small_loader = DataLoader(small_dataset, batch_size=8,
    #                               shuffle=True, num_workers=2,
    #                               collate_fn=detection_collate, drop_last=True)
    small_dataset = torch.utils.data.Subset(train_dataset, range(0, 8))
    ...
    epochs = 100
    # iters_per_epoch = int(len(train_dataset) / 8)
    ...
    for epoch in range(epochs):
        ...
        for step in range(iters_per_epoch):
            ...
            with torch.no_grad():
                for i in range(8):
                    ...
                    pytorch_model.params[f'gamma{i}'] -= learning_rate * grads[f'gamma{i}'].reshape(-1)
                    pytorch_model.params[f'beta{i}'] -= learning_rate * grads[f'beta{i}'].reshape(-1)
        ...
        if epoch % 100 == 0:
            filename = "/home/msis/Desktop/Python/Python_CNN_WeightUpdate/run/epoch_%d.pkl" % (epoch)
    ...
    plt.plot(range(1, epochs + 1), Loss, linestyle='-')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Loss Curve')
    plt.grid(True)
    # plt.show()
    plt.savefig('SoftLightNorm8_100.png')
